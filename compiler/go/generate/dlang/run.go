...
// Package dlang generates D source from a resolved Thrift program.
//
// It is a method-for-method port of
// compiler/cpp/src/thrift/generate/t_d_generator.cc: the emitter builds
...
// parity tests. The package is named dlang, not d, because --gen d is
// the language name the C++ compiler registers and "d" is a poor Go
// package identifier.
package dlang
...
// The option table of THRIFT_REGISTER_GENERATOR(d, "D", ""). The C++
// generator's constructor registers no options ("/* no options yet */")
// and throws "unknown option d:<key>" for anything it is handed; see
// ParseOptions.
func init() {
...
// runner is the registry's view of one parsed --gen d argument.
...
// main.cc restricted to the D generator.
...
func generateOne(program *sema.Program, opts Options) (err error) {
	...
	newGenerator(program, opts).generateProgram()
	return nil
}

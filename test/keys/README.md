...
address (`::ffff:127.0.0.1`): Go refuses to load a certificate with one, and
the Python peer matcher reduces a mapped peer address to its IPv4 form before
comparing, so the entry is not needed.
